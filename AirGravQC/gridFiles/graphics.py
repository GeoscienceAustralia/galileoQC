...
# -*- coding: utf-8 -*-
"""
Collection of functions for creating and manipulating graphics, colormaps and
plots with Python and matplotlib.
...
import AirGravQC.colors # definition of new colormaps
...
def makeColormap(table, name='CustomMap'):
    """
    Return a LinearSegmentedColormap which is subsequently registered with
...
def equalizeColormap(cmap, bins, cdf, name='EqualizedMap'):
    ...
    instead of the image. *This is not strictly speaking the equalization of the 
    colormap itself*.
...
def imshow_hs(data, ax=None, cmap='geosoft', cmap_norm='equalize', hs=True, 
    ...
    if cmap_norm in ['equalize','equalization', 'equalise', 'equalisation']:
        ...
        # colormap is one of the extra ones added by the colors module
        # can add new cmaps by plt.register_cmap(name, cmap)
