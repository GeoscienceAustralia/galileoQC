...
def checkRepeatLines(whizzFiles, channel, repeatLines, x='', z='', xOffset=True):
    ...
    if deltaX == 0:
        print("error found, probably the lines are not in the data.")
        return
    temp_repeats = repeatLines.copy()
        ...
        with h5py.File(filename, 'r') as f:
            ...
            for line in all_flightLines:
                if line in temp_repeats:
                    ...
                    # interpolate data
                    (yOut, _) = util._interpolateLine(xd-xBase[0], yd, xBase-xBase[0])
                    (zOut, _) = util._interpolateLine(xd-xBase[0], zd, xBase-xBase[0])
