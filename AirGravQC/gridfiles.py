...
def read_ers_image(whizzFile, bandout=0):
    ...
    String
        The geographic datum as read from `whizzFile`.
    String
        The geographic projection as read from `whizzFile`.
...
def write_ers_image(imagefile, zz, nullcell=-1.0E32):
    """
    Function to write an array into an ERMapper image.

    Does a simple numpy tofile(), without writing a header (.ers)
    file. See also `write_ers_header`.
...
def write_ers_header(imagefile, headerdict):
    """
    Function to write an ERMapper image header in crude form.
    ...
    imagefile : String
        The name of the image file (includes path, but NOT the .ers suffix).
    headerdict : Dictionary
        As returned by erm.parse_header(imagefile).
