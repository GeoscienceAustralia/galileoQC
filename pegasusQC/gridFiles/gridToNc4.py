...
def gridToNc4(whizz_file, z_chans, grid_space, *, region=None, lines=[], e_chan='', n_chan='', mr_chans=[], 
    ...
    THIS CODE IS IN VERY EARLY DEVELOPMENT - USER BEWARE!

    Parameters
    ----------
    whizz_file : Path or String
    ...
    for z_chan in z_chans:
        ...
        my_data = whizz_to_xarray(whizz_file, z_chan, n_chan=n_chan, e_chan=e_chan, lines=lines, 
        ...
        fname = whizz_file.with_suffix('').name + " " + z_chan

        xd = xr.Dataset(data_vars={fname: my_grid})
