...
1. download and install the galileoQC package;
...
*At the time of writing, galileoQC is in a private repo and this installation will only work if you have access to that repo.*
...
Now everything is installed and you should be able to run galileoQC. To see how this is done, run some of the tutorials from jupyterlab. This should open up in your default browser (a 64-bit browser is strongly recommended).
...
Now everything is installed and you should be able to run `galileoQC`. To see how this is done, run some of the tutorials from jupyterlab:
...
Navigate to galileoQC-main/docs/source/tutorials and run one or more of the tutorial `.ipynb` files, or start a new notebook from the `File` menu. Here is the Odd - Even Analysis notebook from the tutorials.
...
Because we are installing from a downloaded copy of the `galileoQC` repository, upgrading to the latest version involves a few extra steps. First, uninstall `galileoQC` from the whizz environment:

```powershell
(whizz) > python -m pip uninstall galileoQC
```
