...
from galileoQC.gridFiles.oddeven import (_getOddEvenLines, _getTravCtrlLines, _getPlannedLines)
...
def reportSampling(whizzFile, timeChannel='', xChannel='', yChannel=''):
    ...
    with h5py.File(filename, 'r') as f:
        ...
        mean_dt = np.nanmean(time_deltas)
        min_dt = np.nanmin(time_deltas)
        max_dt = np.nanmax(time_deltas)
        std_dt = np.nanstd(time_deltas)
